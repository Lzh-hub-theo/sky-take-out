...
这时候redis的消息备份可以通过后台任务重发。如果如果发送的消息无法到达消息队列的交换机就把redis相应消息删掉，这种方式估计绝大多数消息都会很快地
被程序删掉，不会占据内存很多空间。

然后还有一小部分可能由于交换机无法通过消息的路由键转发到对应的队列，这时候，就可以存入数据库中帮助排查相关的问题，由于这种无法转发到队列的消息
大概率重试过后还是失败，所以需要重新将在redis中扣减的库存恢复，并且返回给前端失败的信息。
